...
class Gen3Pokemon:
	...
	def __expgroup(self, id):
		# Treecko forcefully put in Slow
		if id == 277:
			return "Slow"
	...
	def ek_specific_expgroup(self):
		if self.species['name'].lower() in ["treecko", "grovyle", "sceptile", "voltorb", "wartortle", "flaaffy", "onix", "cubone", 
									  "marowak"]:
			self.expgroup = "Slow"
		elif self.species['name'].lower() in ["dunsparce", "clamperl", "mankey", "primeape", "carvanha", "marill", "azumarill", "omanyte", 
										"gloom", "donphan", "makuhita", "hariyama", "mantine"]:
			self.expgroup = "Medium Slow"
		elif self.species['name'].lower() in ["slugma", "magcargo", "sunflora", "goldeen", "seaking", "dustox", "crawdaunt"]:
			self.expgroup = "Fast"
	...
	def __init__(self, pkm):
		...
		# Convert decrypted bytes to bytearray
		decrypted_bytes = bytearray()
		for section_bytes in decrypted:
			decrypted_bytes.extend(section_bytes)
		
		# Concatenate bytes with bytes
		self.data = self.data[0:32] + decrypted_bytes
		...
		self.item = self.__item(int(struct.unpack('<H', sections['G'][2:4])[0]))
		self.exp = int(struct.unpack('<I', sections['G'][4:8])[0])
		self.expgroup = self.__expgroup(self.species['id'])

		# Emerald Kaizo has toyed with some pokemon's exp group. Update as we find a new one each time...
		self.ek_specific_expgroup()
		...
		for i in range(0, 4):
			...
			pp_section_a = sections['A'][(i + 8)]
			if not isinstance(pp_section_a, int):
